...
$(document).ready(function() {
	...
	$(function() {
	  ...
	  $('.filter-dropdown-menu').click(function(event){
		event.stopPropagation();
	  	...
	  	$(function() {
		$("#table-filter1").on("click", function(a) {
		  $(".form-sorts").toggleClass("table-filter-show");
		  a.stopPropagation()
		});
		$(document).on("click", function(a) {
    ...
    function initializeDateRangePicker(element, form) {
        var start, end;
        var today = moment();

        if (sessionMonth && sessionYear) {
            // Calculate start and end dates based on session values
            start = moment([sessionYear, sessionMonth - 1]); // Month is zero-indexed in moment
            end = moment(start).endOf('month');
            // start = moment().startOf('isoWeek');
		    // end = moment().endOf('isoWeek');
        } else if (element === '.bookingrange1') {
		    // start = moment().startOf('month');
		    // end = today;
		    start = moment().startOf('isoWeek');
		    end = moment().endOf('isoWeek');
		} else if (element === '.bookingrange2') {
		    start = moment().startOf('month');
		    end = today;
		}else if (element === '.bookingrange3') {
            // If initializing for '.bookingrange3', set start date to the beginning and end date to the end of the last month
            start = moment().subtract(1, 'month').startOf('month');
            end = moment().subtract(1, 'month').endOf('month');
         }else if (element === '.bookingrange4') {
            // For '.bookingrange4', set start date to the beginning of the current week and end date to the end of the current week
            start = moment().startOf('isoWeek');
            end = moment().endOf('isoWeek');
        }else if (element === '.bookingrange') {
            start = moment().startOf('isoWeek');
            end = moment().endOf('isoWeek');
        } else {
            // For other elements, set start date to 6 days ago and end date to today
        ...
        // Retrieve selected date range and URL from local storage if available
        var storedStartDate = localStorage.getItem('selectedStartDate');
        var storedEndDate = localStorage.getItem('selectedEndDate');
        var storedURL = localStorage.getItem('storedURL');
        var previousURL = document.referrer;
        if (storedStartDate && storedEndDate && storedURL === previousURL) {
            selectedStartDate = moment(storedStartDate);
            selectedEndDate = moment(storedEndDate);
        } else {
            // Clear stored date range if URL has changed
...
//        }, booking_range);
        ...
        if (!storedStartDate || !storedEndDate || storedURL !== previousURL) {
            // Set the initial date range to last 7 days if not stored in local storage or URL has changed

            if (element === '.bookingrange1') {
	            // selectedStartDate = moment().startOf('month');
	            // selectedEndDate = moment();
	            selectedStartDate = moment().startOf('isoWeek');
		    	selectedEndDate = moment().endOf('isoWeek');
            }else if (element === '.bookingrange2') {
	            selectedStartDate = moment().startOf('month');
	            selectedEndDate = moment();
            } else if (element === '.bookingrange3') {
                ...
                selectedStartDate = moment().subtract(6, 'days');
                selectedEndDate = moment();
            }

            booking_range(selectedStartDate, selectedEndDate);
        }

        $(element).on('apply.daterangepicker', function(ev, picker) {
            selectedStartDate = picker.startDate;
            selectedEndDate = picker.endDate;
            localStorage.setItem('selectedStartDate', selectedStartDate.format('YYYY-MM-DD'));
            localStorage.setItem('selectedEndDate', selectedEndDate.format('YYYY-MM-DD'));
            localStorage.setItem('storedURL', window.location.href); // Store the current URL
            booking_range(selectedStartDate, selectedEndDate);
            submitForm(form); // Submit the form directly after selecting the date range
        });
    }

    // Call the function for each date range picker instance
    initializeDateRangePicker('.bookingrange', '#reportForm');
    initializeDateRangePicker('.bookingrange1', '#clientForm');
    initializeDateRangePicker('.bookingrange2', '#employeeForm');
    initializeDateRangePicker('.bookingrange3', '#dateRangeForm');
    initializeDateRangePicker('.bookingrange4', '#reportForm');
    ...
    $('#sidebar-menu .submenu a').click(function() {
        var previousTab = localStorage.getItem('activeTab');
     ...
     $('a.nav-link').click(function() {
        // Clear stored date range values when navigating between pages
        localStorage.removeItem('selectedStartDate');
        localStorage.removeItem('selectedEndDate');
        localStorage.removeItem('storedURL');
    });
